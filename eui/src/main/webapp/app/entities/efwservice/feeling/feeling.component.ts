...
@Component({
  selector: 'jhi-feeling',
  templateUrl: './feeling.component.html'
})
export class FeelingComponent implements OnInit, OnDestroy {
  feelings: IFeeling[];
  currentAccount: any;
  ...
  loadAll() {
    this.feelingService
      ...
      .subscribe(
        (res: HttpResponse<IFeeling[]>) => this.paginateFeelings(res.body, res.headers),
  ...
  trackId(index: number, item: IFeeling) {
    return item.id;
  }
  ...
  protected paginateFeelings(data: IFeeling[], headers: HttpHeaders) {
